...
class UniPuncModel(BaseFairseqModel):
    ...
    def add_args(cls, parser: ArgumentParser):
        ...
        parser.add_argument(
            "--pretrain-model",
            type=str,
            help="The pretrained text encoder model of the UniPunc, "
        ...
        parser.add_argument("--W2V-DIM", type=int, default=768, help="The w2v dim")
        parser.add_argument(
            "--header-model",
            type=str,
            help="The multihead attention model"
            "We provide several header models in fairseq_code/models/headers"
        ...
        parser.add_argument(
            "--head-layer-number", type=int, help="The layer number of header"
        )
        parser.add_argument(
            "--freeze-w2v", action="store_true", help="Freeze wav2vec module"
        )
        parser.add_argument(
            "--grad-scale-wav", type=int, default=None, help="Grad scale wav"
        )
        parser.add_argument(
            "--wav-mask-prob",
            type=float,
    ...
    def build_model(cls, args, task):
        pretrained_model = pretrain_model_register.build(
            args.pretrain_model, args, task
        )
        ...
        return cls(
            ...
            pretrained_model,
            header_model,
            wav2vec_ckpt=wav2vec_ckpt,
            # mask_prob=getattr(args, "wav_mask_prob", 0),
            mask_prob=args.wav_mask_prob,
            pad_index=task.dictionary.pad_index,
    ...
    def _get_w2v_feature(self, src_tokens, src_lengths):
        ...
        w2v_feature, padding_mask = self.wav2vec_model.extract_features(
            src_tokens, padding_mask, mask=self.mask_wav and self.training
        )  # only mask when training
    ...
    def forward(
        ...
        if self.ignore_wav:
            src_feature = self.pretrained_model(
                src_tokens, attention_mask=attention_mask, src_lengths=src_lengths
            )
            ...
            if self.virtual_embedding and is_audio:
                ...
                expanded_virtual_embedding = self.virtual_embedding.expand(
                    [bsz, VIRTUAL_EMB_LEN, W2V_DIM]
                ).to(w2v_feature.dtype)
                ...
                w2v_feature = (
                    w2v_feature * is_audio_masking
                    + expanded_virtual_embedding_padded * (~is_audio_masking)
                )

                virtual_size = VIRTUAL_EMB_LEN * torch.ones([bsz], dtype=torch.int) * (
                    ~is_audio
                ) + src_lengths * (is_audio)
            ...
            res = self.header_model(
                src_feature, w2v_feature, src_padding_mask, encoder_padding_mask
            )
