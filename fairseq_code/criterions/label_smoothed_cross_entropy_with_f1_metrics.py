...
@register_criterion(
    "label_smoothed_cross_entropy_with_f1_metrics",
    LabelSmoothedCrossEntropyCriterionConfig,
)
class LabelSmoothedCrossEntropyCriterionWithF1Metrics(
    LabelSmoothedCrossEntropyCriterion
):
    def forward(self, model, sample, reduce=True):
        ...
        if net_output[1] is None:
            loss, nll_loss = self.compute_loss(
                model, net_output[0], sample, reduce=reduce
            )
        else:
            loss = net_output[1]
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        predict = np.concatenate(
            [logging_output["predict"] for logging_output in logging_outputs]
        )
        target = np.concatenate(
            [logging_output["target"] for logging_output in logging_outputs]
        )
        ...
        for column in df.columns:
            for index in df.index:
                metrics.log_scalar(f"{column}-{index}", df.loc[index, column], round=3)
